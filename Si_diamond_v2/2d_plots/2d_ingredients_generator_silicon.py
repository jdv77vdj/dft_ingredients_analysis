...
### Define systems below
systems['Si']     = {'lattice_const': 10.860000, 'kxoff': .375, 'kyoff': .375, 'kzoff': .375, 'yp': 0.0, 'z': 4.072}    # needs NO /2 in ELFCAR due to spin
     ...
     #       Rho[k] = float(RhoV[k])/Vcell
        ...
        if path==False:
            if nuclei==False:
                ...
                if twodim==True:
                    # write to file, but select only one z-value and print empty lines, whenever y changes (for 2D-projection)
                    plot_dir = os.path.join(BASE_PATH,system)
                    ...
                    with open(output.name, 'w', encoding=output.encoding) as o:
                        ...
                        for k in range(0,NGX*NGY):
                            ...
                            o.write(outstring)
                    print("Done writing in 2D, have a coffee :)")
