...
### Must have the file named generate_Fx.py in the same folder
...
systems['T']   = {'lattice_const': 10.860000, 'kxoff': .375, 'kyoff': .375, 'kzoff': .375, 'yp': 0.0}    # needs NO /2 in ELFCAR due to spin
#systems['X']   = {'lattice_const': 10.860000, 'kxoff': .375, 'kyoff': .375, 'kzoff': .375, 'yp': 0.0}    # needs NO /2 in ELFCAR due to spin
#systems['H']   = {'lattice_const': 10.860000, 'kxoff': .375, 'kyoff': .375, 'kzoff': .375, 'yp': 0.0}    # needs NO /2 in ELFCAR due to spin
     ...
     #       Rho[k] = float(RhoV[k])/Vcell
        ...
        if path==True:
            # write only path to file
                plot_dir = os.path.join(BASE_PATH, system)
                outfile = os.path.join(plot_dir, 't2_1d_data_path_ae.dat') #data_path.dat')
                output = open(outfile, 'w')
                with open(output.name, 'w', encoding=output.encoding) as o:
                    o.write(f'# System: {system}\tNGX: {NGX}\tNumber of points: {len_ELF}\n')
                    o.write(f'# Nr \t x \t\t y \t \t z in A \t rho \t \t rs \t \t t \t \t s \t \t alpha \t \t FxPBE \t \t FxSCAN \t Fxr2SCAN \t FxLAK \t \t FcPBE \t \t FcSCAN \t Fcr2SCAN \t FcLAK \t FcLDA \n\n')
                    # Define path
                    r1 = np.array([4.072, 4.072, 4.072]) # Interstitial position
                    #r0 = np.array([4.072, 1.357, 4.072]) # t1 in the plots
                    r0 = np.array([5.4295, 2.7145, 4.072]) # t2 in the plots
                    Npath = 200
