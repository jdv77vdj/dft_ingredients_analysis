...
def plot_defect(metal_name, defect_name, limits):
   ...
   # ratio_pbe, ratio_scan, ratio_r2scan, ratio_lak, ratio_lda = [d/p for d, p in zip(exc_def, exc_pr)]
    ...
    reference = ratio_lda
    lwidth=3;
    ax_left.plot(x, ratio_pbe, color="tab:blue",   lw=lwidth, ls="-", label="PBE")
    ax_left.plot(x, ratio_scan, color="tab:orange", lw=lwidth, ls="-", label="SCAN")
    ax_left.plot(x, ratio_r2scan, color="tab:green",  lw=lwidth, ls="-", label="r2SCAN")
    ax_left.plot(x, ratio_lak, color="tab:cyan",   lw=lwidth, ls="-", label="LAK")
    ax_left.plot(x, ratio_lda, color="tab:purple", lw=lwidth, ls="-", label="LDA")
    
    ax_left.axhline(0.0, color="k", lw=0.8, ls="--")
    ax_left.set_ylabel(r"$1/ R_{xc}$", fontsize=20)
...
#        )
    ...
    axins3 = inset_axes(ax_left, width="18%", height="30%", loc="lower right")
...
#    ax_right.plot(x, drs, color='tab:red',   lw=lwidth, label=r"$\Delta r_s$")
    ...
    ax_right.plot(x, s_pr,  color='tab:blue',  lw=lwidth, ls="--", label=r"$s_{\mathrm{pris}}$")
    ax_right.plot(x, a_pr,  color='tab:green', lw=lwidth, ls="--", label=r"$\alpha_{\mathrm{pris}}$")
    ax_right.plot(x, rs_pr, color='tab:red',   lw=lwidth, ls="--", label=r"$r_{s,\mathrm{pris}}$")
    
    ax_right.plot(x, s_def,  color='tab:blue',  lw=lwidth, label=r"$s_{\mathrm{def}}$")
    ax_right.plot(x, a_def,  color='tab:green', lw=lwidth, label=r"$\alpha_{\mathrm{def}}$")
    ax_right.plot(x, rs_def, color='tab:red',   lw=lwidth, label=r"$r_{s,\mathrm{def}}$")
    ...
    plt.savefig(f"Ingr_{metal_name}_{defect_name}.pdf", dpi=300, bbox_inches="tight")
    plt.show()
    print(f"Ingr_{metal_name}_{defect_name}.pdf")
    plt.close()
    

#plot_limits = {
#    "T": {"ylim_left": (0, 1.1), "ylim_right": (0, 4.5), "xlim_left": (0.0, 1.2), "xlim_right": (0.0, 2.8), "Ef_range": (0.0, 5.0), "reg_x1":(0.0, 0.16), "reg_x2":(0.16, 0.82), "reg_x3":(0.82, 1.33),"reg_x4":(1.33, 2.53)},
...
plot_limits = {
    "T": {"ylim_left": (0, 1.1), "ylim_right": (0, 4.5), "xlim_left": (0.0, 1.2), "xlim_right": (0.0, 2.8), "Ef_range": (0.0, 5.0), "reg_x1":(0.0, 0.16), "reg_x2":(0.16, 0.50), "reg_x3":(0.50, 2.21),"reg_x4":(2.21, 2.53)},
    "X": {"ylim_left": (-0.005, 0.002), "ylim_right": (-0.11, 0.15), "xlim_left": (0.3, 2.5), "xlim_right": (0.51, 2.8), "Ef_range": (0.0, 5.0),"reg_x1":(0.0, 0.20), "reg_x2":(0.20, 0.48)},
    "H": {"ylim_left": (-0.013, 0.017), "ylim_right": (-8.5, 0.6), "xlim_left": (0.3, 2.5), "xlim_right": (0.51, 2.8), "Ef_range": (0.0, 5.0),"reg_x1":(0.0, 0.29), "reg_x2":(0.29, 0.62)}}
