...
def plot_2d_ingredients(metal):
    ...
    #all_s = np.concatenate([S_pr.flatten(), S_def.flatten()])
    #vmin_s = np.percentile(all_s, 25)
    #vmax_s = np.percentile(all_s, 75)
...
#    vmax_s = max(S_pr.max(), S_def.max())
    ...
    levels_rs = np.linspace(0, 3.5, 100)
...
#    im4.set_rasterized(True)
    ...
    # =========================================================
#
#    levels_a = np.linspace(0, 4.0, 100)
#    im5 = axes[1,0].contourf(
#        X_pr, Y_pr, A_pr,
#        levels=levels_a,
#        extend='max'
#        )
...
#    im6.set_rasterized(True)
    ...
    levels_a = np.linspace(0, 3.0, 100)
...
#    im6.set_rasterized(True)
    ...
    axes[0,0].set_title(f"{metal} pristine", fontsize=18)
    axes[0,1].set_title(f"{metal} vacancy", fontsize=18)
