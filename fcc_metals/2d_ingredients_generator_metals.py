            ...
            if nuclei==False:
                ...
                if twodim==True:
                    ...
                    with open(output.name, 'w', encoding=output.encoding) as o:
                        ...
                        #z_target = 4.018
                        #z_target = 4.00 # this works better for s
                        z_target = properties['z']
                        #z_target = properties['z_frac'] * lattice_const
                        #z_target = 4.1
                        z_plane_index = np.argmin(np.abs(np.array(z) - z_target))
                        z_offset = (z_plane_index // (NGX*NGY)) * NGX * NGY
                        zfix = z[z_offset]
                        # previous code
                        #z_offset = math.floor(0.125*NGZ)*NGX*NGY      # select inex of z-value
                        #zfix = z[z_offset]
            ...
            # write only path to file
                ...
                with open(output.name, 'w', encoding=output.encoding) as o:
                    ...
                    for k in range(0,math.ceil(NGX/2)):
                        ...
                        o.write(outstring)
                        i = i+1
#                    for k in range(0,math.ceil(NGX/4)):
#                        kloc0 = k*(1+NGX) + math.ceil(NGX*(kxoff) + NGX*(NGY*(kyoff)) )#+ NGX*NGY*(NGZ*kzoff) )  # start 0.25 before vacyncy; vacancy at (kxoff,kyoff,kzoff).
#                        kloc = math.ceil(NGX/4) - kloc0
#                        if Rho[kloc] > 0.0:
#                            s = calc_s_single_k(kloc, NGX, dx, Rho)
#                            alpha = math.sqrt((1./float(ELF[kloc])-1.))
#                        else:
#                            s = -1.0; alpha = -1.0
#                        xloc = x[kloc] - (kxoff)*lattice_const # shift x (and y) to start at zero for easier plots (shifts vacancy to (0.25,0.25,z) )
#                        yloc = y[kloc] - (kyoff)*lattice_const
#                        #o.write(f'{i}\t{x[kloc]:.6f}\t{y[kloc]:.6f}\t{z[kloc]:.6f}\t{Rho[kloc]:.6f}\t{rs[kloc]:.6f}\t{t[kloc]:.6f}\t{s[kloc]:.6f}\t{alpha[kloc]:.6f}\t{kloc}\n')
#                        outstring = calc_write_single_k(i, xloc, yloc, z[kloc], Rho[kloc], s, alpha)
#   #                     o.write(outstring)
#                        i = i+1
#                    for k in range(0,math.ceil(NGX/4)):
#                        kloc = k*(1+NGX) + math.ceil(NGX*(kxoff) + NGX*(NGY*(kyoff)) )#+ NGX*NGY*(NGZ*kzoff) )  # start 0.25 before vacyncy; vacancy at (kxoff,kyoff,kzoff).
#                        if Rho[kloc] > 0.0:
#                            s = calc_s_single_k(kloc, NGX, dx, Rho)
#                            alpha = math.sqrt((1./float(ELF[kloc])-1.))
#                        else:
