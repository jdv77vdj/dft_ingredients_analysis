...
def plot_metal(metal_name, limits, ax_1, ax_left, ax_mid, ax_right):
...
#        dist[i] = dist[i-1] + np.linalg.norm(path_points[i] - path_points[i-1])
#    
    coords = data_pr[:, 1:4]  # x,y,z
    dist = np.zeros(len(coords))

    for i in range(1, len(coords)):
        dist[i] = dist[i-1] + np.linalg.norm(coords[i] - coords[i-1])
    ...
    #delta_exc = [d - (31./32)*p for d, p in zip(exc_def, exc_pr)]
    
    #delta_exc = [(d - p)/np.abs(p) for d, p in zip(exc_def, exc_pr)]
    
    delta_exc = [
    (d - p) - (exc_def[4] - exc_pr[4])   # subtract LDA
    for d, p in zip(exc_def, exc_pr)]
    
    print(f"Ratio LAK of {metal_name}")
    # Checks if ANY element is less than 0
    has_negatives = (ratio_lak < 0).any()
    print(f"Ratio LAK of {metal_name} has negatives:", has_negatives)  # Output: True
    
#    exc_pr .* ratio_lda > exc_def
...
#    ax_left.axvspan(x4[0], x4[1], color=color_base, alpha=0.20, zorder=0)
    ...
    ax_1.plot(y, delta_exc[0], color="tab:blue",   lw=lwidth, ls="-", label="PBE")
    ax_1.plot(y, delta_exc[1], color="tab:orange", lw=lwidth, ls="-", label="SCAN")
    ax_1.plot(y, delta_exc[2], color="tab:green",  lw=lwidth, ls="-", label="r2SCAN")
    ax_1.plot(y, delta_exc[3], color="tab:cyan",   lw=lwidth, ls="-", label="LAK")
   # ax_1.plot(y, delta_exc[4], color="tab:purple", lw=lwidth, ls="-", label="LDA")
    
    ax_1.axhline(0.0, color="k", lw=0.8, ls="--")
    ax_1.set_ylabel(r"$e_{xc}^{def} - e_{xc}^{pris}$", fontsize=24)
    ax_1.set_ylim(-0.05,0.05)
    #ax_1.set_xlabel(r"$r(\AA)$", fontsize=20)
...
#   ax_left.set_xlim(2.2,4.00)
    ...
    axins3 = inset_axes(ax_1, width="20%", height="30%", loc="lower right")
...
#    ax_left.axvspan(x4[0], x4[1], color=color_base, alpha=0.20, zorder=0)
    ...
    #ax_left.set_xlabel(r"$r(\AA)$", fontsize=20)
...
#    ax_left.set_xlim(2.2,4.00)
    ...
    axins3 = inset_axes(ax_left, width="20%", height="30%", loc="upper right")
    x_f = np.arange(len(functionals))
    axins3.bar(x_f, energies, color=colors_func, alpha=1, width=0.7)
    axins3.set_zorder(200)
    axins3.axhline(0, color="k", lw=1.2) # zero reference line
...
#    ax_mid.set_xlim(y.min(), y.max())
#    ax_right.set_xlim(y.min(), y.max())

### Ticks :)
#    xticks = [y[0], x_seg1, x_seg2, y[-1]]
#    xticklabels = [r"$r_1$", r"$r_2$", r"$r_3$", r"$r_4$"]
#
...
#        ax.set_xticklabels(xticklabels)
    ...
    x_r1 = 0.5 * (xmin + x_seg1)
    x_r2 = 0.5 * (x_seg1 + x_seg2)
    x_r3 = 0.5 * (x_seg2 + xmax)

    for ax in [ax_1, ax_left, ax_mid, ax_right]:
        ymin, ymax = ax.get_ylim()
        
        # place labels slightly below top
        y_text = ymax - 0.08*(ymax - ymin)

        ax.text(x_r1, y_text, r"$a$", ha='center', va='top', fontsize=18)
        ax.text(x_r2, y_text, r"$b$", ha='center', va='top', fontsize=18)
        ax.text(x_r3, y_text, r"$c$", ha='center', va='top', fontsize=18)
    
    for ax in [ax_1, ax_left, ax_mid, ax_right]:
        ax.axvline(x_seg1, color="k", lw=1.5, ls="--", alpha=0.7)
        ax.axvline(x_seg2, color="k", lw=1.5, ls="--", alpha=0.7)
...
def plot_group(metals, filename):
    ...
    figsize=(24, 20),
    sharex='col',
    gridspec_kw={"width_ratios":[1.5,1.5,1.5,1.5]}
    )
    ...
    for i, metal in enumerate(metals):
        ...
        plot_metal(metal, plot_limits[metal], ax_1, ax_left, ax_mid, ax_right)
        ...
        if i == 0:
            ...
            handles_ax1, labels_ax1 = ax_1.get_legend_handles_labels()
