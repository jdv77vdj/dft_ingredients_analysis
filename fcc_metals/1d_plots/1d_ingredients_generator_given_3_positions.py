...
### Script to generate ingredients given 3 position vectors.
### Example of use: python 1d_ingredients_generator_given_3_positions.py -a -p
...
#systems['Al']    = {'lattice_const': 8.036000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.018}
#systems['Alv']   = {'lattice_const': 8.036000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.018}
systems['Cu']    = {'lattice_const': 7.190000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.595}
systems['Cuv']   = {'lattice_const': 7.190000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.595}
systems['Ag']    = {'lattice_const': 8.124000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.062}
systems['Agv']   = {'lattice_const': 8.124000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.062}
systems['Au']    = {'lattice_const': 8.124000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.062}
systems['Auv']   = {'lattice_const': 8.124000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.062}
systems['Ni']    = {'lattice_const': 7.016000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.508}
systems['Niv']   = {'lattice_const': 7.016000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.508}
systems['Pd']    = {'lattice_const': 7.752000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.876}
systems['Pdv']   = {'lattice_const': 7.752000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.876}
systems['Pt']    = {'lattice_const': 7.826000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.913}
systems['Ptv']   = {'lattice_const': 7.826000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 3.913}
systems['Pb']    = {'lattice_const': 9.824000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.912}
systems['Pbv']   = {'lattice_const': 9.824000, 'kxoff': .5, 'kyoff': .5, 'kzoff': .5, 'yp': 0.25, 'z': 4.912}
...
def rewrite_ELFCAR(nuclei=False, path=False, twodim=False, xc_func='lda'):    #TODO: nuclei=False, when remove nuclei is enabled
    for system, properties in systems.items():
        ...
        len_ELF = int(len(ELF)/2)   # use this line for spin-UNpolarized systems
     ...
     #       Rho[k] = float(RhoV[k])/Vcell
            ...
            if Rho[k] > 0.0:
                ...
                s_field[k] = calc_s_single_k(k, NGX, dx, Rho)
                alpha_field[k] = math.sqrt((1./float(ELF[k]) - 1.))
            else:
                s[k] = -1.0; alpha[k] = -1.0
                s_field[k] = -1.0
                alpha_field[k] = -1.0
        ...
        if path==True:
            # write only path to file
                plot_dir = os.path.join(BASE_PATH, system)
                outfile = os.path.join(plot_dir, '1d_data_path_ae.dat') #data_path.dat')
                output = open(outfile, 'w')
                with open(output.name, 'w', encoding=output.encoding) as o:
                    o.write(f'# System: {system}\tNGX: {NGX}\tNumber of points: {len_ELF}\n')
                    o.write(f'# Nr \t x \t\t y \t \t z in A \t rho \t \t rs \t \t t \t \t s \t \t alpha \t \t FxPBE \t \t FxSCAN \t Fxr2SCAN \t FxLAK \t \t FcPBE \t \t FcSCAN \t Fcr2SCAN \t FcLAK \t FcLDA \n\n')
                    # Define path, use a visualization tool like ASE or VESTA to see what positions to use
                    r1 = np.array([0, a_lat, a_lat]) # Al 1
                    r2 = np.array([a_lat, a_lat, a_lat]) # Al 2, vacancy is here D:
                    r3 = np.array([a_lat/2, a_lat/2, a_lat]) # Al 3
                    r4 = (r2+r1)/2  # mid point
                    
                    
                    Npath = 200
                    ts = np.linspace(0, 1, Npath)
                    #path_points = r0 + ts[:,None]*(r1-r0)
                    # 3 segments
                    path12 = r1 + ts[:, None] * (r2 - r1)
                    path23 = r2 + ts[:, None] * (r3 - r2)
                    #path31 = r3 + ts[:, None] * (r1 - r3)
                    path41 = r3 + ts[:, None] * (r4 - r3)

                    # concatenate (avoid duplicating endpoints)
                    path_points = np.vstack([
                        path12,
                        path23[1:],   # remove duplicate r2
                        #path31[1:]    # remove duplicate r3
                        path41[1:]
                    ])

                    Rho_field   = np.array(Rho)
                    s_field     = np.array(s_field)
                    alpha_field = np.array(alpha_field)
                    ...
                    for i, point in enumerate(path_points):
                        ...
                        # interpolate
                        rho_p = trilinear_interp(Rho_field, xp, yp, zp, dx, NGX)
                        s_p   = trilinear_interp(s_field, xp, yp, zp, dx, NGX)
                        a_p   = trilinear_interp(alpha_field, xp, yp, zp, dx, NGX)    



                        outstring = calc_write_single_k(
                            i,
                            xp,
                            yp,
                            zp,
...
def trilinear_interp(field, xp, yp, zp, dx, NGX):
    # fractional index
    fx = xp / dx
    fy = yp / dx
    fz = zp / dx

    ix = int(np.floor(fx)) % NGX
    iy = int(np.floor(fy)) % NGX
    iz = int(np.floor(fz)) % NGX
    ...
    def idx(i, j, k):
        return (i % NGX) + NGX*(j % NGX) + NGX*NGX*(k % NGX)

    # 8 corners
    c000 = field[idx(ix,   iy,   iz)]
    c100 = field[idx(ix+1, iy,   iz)]
    c010 = field[idx(ix,   iy+1, iz)]
    c110 = field[idx(ix+1, iy+1, iz)]
    c001 = field[idx(ix,   iy,   iz+1)]
    c101 = field[idx(ix+1, iy,   iz+1)]
    c011 = field[idx(ix,   iy+1, iz+1)]
    c111 = field[idx(ix+1, iy+1, iz+1)]
