...
def plot_2d_ingredients(defect):
    ...
    delta_exc = [d - p for d, p in zip(exc_def, exc_pr)]
...
#    # e_xc differences
    D_PBE = delta_exc[0].reshape(NGY, NGX)
    D_SCAN = delta_exc[1].reshape(NGY, NGX)
    D_R2SCAN = delta_exc[2].reshape(NGY, NGX)
    D_LAK = delta_exc[3].reshape(NGY, NGX)
    
    
    # NEW STUFF
    dx = (x_pr.max() - x_pr.min()) / NGX
    dy = (y_pr.max() - y_pr.min()) / NGY
    dA = dx * dy
    
    total_exc = [np.sum(d) * dA for d in delta_exc]
    PBE_total, SCAN_total, R2SCAN_total, LAK_total, LDA_total = total_exc
    
    pos_exc = [np.sum(np.where(d > 0, d, 0)) * dA for d in delta_exc]
    neg_exc = [np.sum(np.where(d < 0, d, 0)) * dA for d in delta_exc]
...
#    axes[2].set_title(r"$\delta \alpha$", fontsize=18)
    ...
    plt.savefig(
        f"{defect}_delta_exc_2d_ingredients.pdf",
        dpi=600,
        bbox_inches="tight"
    )
    ...
    labels = ["PBE", "SCAN", "r2SCAN", "LAK", "LDA"]
    plt.figure()
    plt.bar(labels, total_exc)
    plt.axhline(0, color='k')
    plt.ylabel(r"$\Delta E_{xc}^{2D}$")
    plt.title(defect)
    plt.savefig(f"{defect}_deltaExc_bar.pdf")
    plt.close()
